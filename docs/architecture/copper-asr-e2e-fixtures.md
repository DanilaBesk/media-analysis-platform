...
- `corrupt_audio`: invalid Ogg-named bytes for failed-run diagnostics. Target and live diagnostic is `asr_invalid_audio`.
...
The Telegram E2E command requires the local compose API, transcription worker, and CopperASR service to be running. It creates a throwaway Telegram channel account, uploads the `short_voice` fixture as a voice message, asserts inbox materialization and current materials surface persistence, launches processing through the atomic collection processing-run endpoint, verifies captured membership detaches at launch, verifies worker progress/finalize events, downloads transcript and run manifest bytes, records a result artifact surface, proves duplicate delivery is blocked by that surface, and proves later inbox membership survives successful delivery.
...
Latest benchmark evidence from the warm existing compose runtime after applying the local compose CPU quota `COPPER_ASR_LOCAL_CPUS=4.0`: the 960.006 second representative voice finished successfully in 165.319 seconds of run wall time with provider `copperasr`, model `Copperside/CoppersideASR`, and local consumer caps `COPPER_ASR_ONNX_NUM_THREADS=2`, `COPPER_ASR_TORCH_NUM_THREADS=2`, `COPPER_ASR_TORCH_INTEROP_THREADS=1`, `COPPER_ASR_FFMPEG_THREADS=1`, and compose `nano_cpus=4000000000`. CopperASR exposes runtime processing telemetry (`vad_s=98.804`, `asr_inference_s=57.919`, `vad_segment_count=56`, `chunk_count=56`); `copper-asr` stayed within threshold at 413.73% max CPU and 297.925% average CPU while worker/API stayed near idle. The benchmark artifact therefore passes thresholds without changing benchmark semantics or editing the external CopperASR submodule.
