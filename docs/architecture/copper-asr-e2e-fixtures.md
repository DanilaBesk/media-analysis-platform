...
Latest benchmark evidence after rebuilding the CopperASR runtime image from `vendor/copper-asr` commit `5184cd4452ac45f0d93fb3e00b6bae005cb597e5`: the 960.006 second representative voice finished successfully in 160.247 seconds of run wall time with provider `copperasr`, model `Copperside/CoppersideASR`, and local consumer caps `COPPER_ASR_ONNX_NUM_THREADS=2`, `COPPER_ASR_TORCH_NUM_THREADS=2`, `COPPER_ASR_TORCH_INTEROP_THREADS=1`, `COPPER_ASR_FFMPEG_THREADS=1`. CopperASR exposes runtime processing telemetry (`vad_s=70.284`, `asr_inference_s=84.225`, `vad_segment_count=56`, `chunk_count=56`), but `copper-asr` still reached 892.62% max CPU while averaging 446.365%; worker/API stayed near idle. The benchmark artifact therefore keeps thresholds failed and links blocker `media-b8s.2.10` for remaining CopperASR max-CPU pressure.
