...
Latest benchmark evidence after the CopperASR submodule update: the 960.006 second representative voice finished successfully in 105.187 seconds of run wall time with provider `copperasr`, model `Copperside/CoppersideASR`, and local consumer caps `COPPER_ASR_ONNX_NUM_THREADS=2`, `COPPER_ASR_TORCH_NUM_THREADS=2`, `COPPER_ASR_TORCH_INTEROP_THREADS=1`, `COPPER_ASR_FFMPEG_THREADS=1`. CopperASR now exposes runtime processing telemetry (`vad_s=43.159`, `asr_inference_s=56.944`, `vad_segment_count=56`, `chunk_count=56`), but `copper-asr` still reached 875.05% max CPU while averaging 412.46%; worker/API stayed near idle. The benchmark artifact therefore keeps thresholds failed and links blocker `media-b8s.2.10` for remaining CopperASR max-CPU pressure.
