...
Latest benchmark evidence after rebuilding the CopperASR runtime image from the external submodule path `external/copper-asr` and applying the local compose CPU quota `COPPER_ASR_LOCAL_CPUS=4.0`: the 960.006 second representative voice finished successfully in 200.28 seconds of run wall time with provider `copperasr`, model `Copperside/CoppersideASR`, and local consumer caps `COPPER_ASR_ONNX_NUM_THREADS=2`, `COPPER_ASR_TORCH_NUM_THREADS=2`, `COPPER_ASR_TORCH_INTEROP_THREADS=1`, `COPPER_ASR_FFMPEG_THREADS=1`, and compose `nano_cpus=4000000000`. CopperASR exposes runtime processing telemetry (`vad_s=120.219`, `asr_inference_s=76.999`, `vad_segment_count=56`, `chunk_count=56`); `copper-asr` stayed within threshold at 409.15% max CPU and 305.774% average CPU while worker/API stayed near idle. The benchmark artifact therefore passes thresholds without changing benchmark semantics or editing the external CopperASR submodule.
