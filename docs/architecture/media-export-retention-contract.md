...
Audio or video produced for export is a transient managed object whose body follows `MEDIA_OBJECT_RETENTION_DAYS`, default `7`, independently from delivery retry and access windows. Each export job pins an immutable output profile and presentation snapshot at creation. YouTube audio waits for stable provider title and performer metadata before snapshot creation; migration `0009` requeues pre-existing YouTube assets without performer metadata. New audio profiles produce M4A containers encoded with AAC; historical OGG/Opus jobs retain their original profile across retry, and video exports remain MP4 with AAC audio. API finalization rejects MIME or filename that contradicts the pinned profile, and a cancellation won under the storage lock cannot retain an output identity, duration, or presentation freeze from the losing success attempt. Export output persists long enough for restart-safe Telegram or Web delivery, retry, and audit lineage. If a channel already has the same verified SHA-256 and size, finalize reuses that canonical stored-object identity, extends its expiry monotonically, and attaches an independent delivery pin; the redundant attempt-owned promoted body is removed after the transaction or by reconciliation. Telegram resolves a private, container-reachable presigned URL through an authenticated internal API route, streams the response into an anonymous disk-backed file, and lets the Bot API consume that file in bounded chunks; the adapter never buffers a contract-valid export wholly in RAM. Trusted download MIME plus presentation metadata controls Telegram rendering: new YouTube `audio/mp4` uses `send_audio` with snapshotted title, performer, and duration and no generic caption; historical `audio/ogg`, `audio/opus`, and `application/ogg` use `send_voice`; generic `audio/*` uses `send_audio`; non-audio retains `send_document`. The anonymous handle is closed in `finally` and by the operating system on process exit. Telegram acknowledges delivery only after a successful send. Web link issuance is not an acknowledgement; Web access is issued for `EXPORT_WEB_ACCESS_TTL_HOURS` from successful completion and never extends the body beyond its media-retention expiry. Pending or failed delivery is bounded by `EXPORT_DELIVERY_TTL_HOURS` and `EXPORT_DELIVERY_MAX_ATTEMPTS`; expiry releases its retention pin without redefining the stored body's retention window.
...
Raw provider format ids are worker details. New M4A/AAC audio profiles accept semantic values `64`, `96`, `128`, `192`, `256`, and `320` kbps; historical OGG/Opus profiles remain immutable job facts and are not silently upgraded during retry. Public video variants use bounded semantic quality values.
...
- Required commands are `go test ./...` in `apps/api`, `uv run pytest` for Python workers/Telegram, Web tests/build, `grace lint --path . --assertions current` plus `grace status --path . --json`, focused retention/export runtime script, full repository test gate, and Compose smoke.
