...
| CopperASR corrupt-audio classification follow-up | Closed by CopperASR `main` commit `f880151cfc57e082a94c028fb0d7483ccc1a921b`, rebuilt from `external/copper-asr`. The strict live command `python3 infra/scripts/copper-asr-failure-e2e.py --json --require-invalid-audio` records `diagnostic_codes=["asr_invalid_audio"]` for both corrupt and retry runs, publishes only `run_diagnostics` and `run_manifest`, and leaves cancellation at `canceled` with `artifact_count=0`. The manifest no longer accepts `asr_unexpected_runtime_error` for the corrupt fixture. |
...
| `asr_invalid_audio` | Invalid or unsupported audio must fail the transcription run with analysis_run diagnostics and run policy artifacts, never transcript artifacts or a stuck active card. | worker RED/GREEN regression; `python3 infra/scripts/copper-asr-failure-e2e.py --json` and `python3 infra/scripts/copper-asr-failure-e2e.py --json --require-invalid-audio` cover live corrupt-audio failed run and retry with `asr_invalid_audio`. | Strict invalid-audio proof passes on CopperASR `f880151cfc57e082a94c028fb0d7483ccc1a921b`; no temporary accepted live diagnostic remains in the manifest. |
