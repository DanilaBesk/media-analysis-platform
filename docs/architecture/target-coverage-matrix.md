...
| CopperASR corrupt-audio classification follow-up | The target corrupt-audio diagnostic is `asr_invalid_audio`, but the live CopperASR runtime currently returns HTTP 500 provider code `unexpected_runtime_error` for the corrupt fixture. The manifest accepts both `asr_invalid_audio` and `asr_unexpected_runtime_error` for live failure E2E until `media-b8s.2.9` normalizes provider or platform preflight classification. |
...
| CopperASR E2E fixtures are deterministic. | Manifest declares short voice, representative long voice, corrupt audio, cancellation voice, and artifact download cases under `copper_asr_e2e`; `infra/scripts/copper-asr-e2e-harness.py` validates hashes and can copy object-store bytes into a deterministic bucket/key tree; `infra/scripts/copper-asr-api-web-mcp-e2e.py` consumes the short voice case against the live stack; `infra/scripts/copper-asr-failure-e2e.py` consumes the corrupt and cancellation cases. | `test_copper_asr_e2e_fixture_manifest_covers_required_inputs_and_hashes`; `test_copper_asr_e2e_harness_reports_deterministic_fixture_plan`; `python3 infra/scripts/copper-asr-api-web-mcp-e2e.py --json`; `python3 infra/scripts/copper-asr-failure-e2e.py --json`. | `media-b8s.2.2`, `media-b8s.2.6`, and `media-b8s.2.7` must consume the remaining Telegram, inventory, and benchmark proof. |
...
| `asr_invalid_audio` | Invalid or unsupported audio must fail the transcription run with analysis_run diagnostics and run policy artifacts, never transcript artifacts or a stuck active card. | worker RED/GREEN regression; `python3 infra/scripts/copper-asr-failure-e2e.py --json` covers live corrupt-audio failed run and retry with accepted live diagnostic codes. | `media-b8s.2.9` must remove the temporary acceptance of `asr_unexpected_runtime_error` for corrupt audio. |
| `asr_unexpected_runtime_error` | Unexpected CopperASR provider failures must still finalize the run, record ASR diagnostics, and publish only policy artifacts instead of surfacing generic backend-unavailable loops. | live corrupt-audio E2E currently observes this code from CopperASR HTTP 500 and verifies no transcript artifacts are published. | Provider/preflight classification follow-up in `media-b8s.2.9`. |
