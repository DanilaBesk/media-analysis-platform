...
| --- | --- | --- |
| Coverage matrix maps each source-plan requirement to implementation proof, test proof, and QA proof. | This file maps data reset, tables, API operations, user flows, failure modes, removed-surface rules, non-goals, and app boundaries. `media-7f3.10.2` added storage/API proof. `media-7f3.10.3` added adapter, worker, and compose runtime proof. `media-7f3.10.4` added runnable inventory/no-legacy gates. | `media-7f3.11.*` must challenge any stale row assignments before final readiness. |
...
| Runtime target vocabulary | `infra/scripts/compose-smoke.sh --check-config` requires `/internal/v1/channel-accounts`, `channel_account_id`, `/v1/media-assets`, `/v1/selection-snapshots`, `selection_snapshot_id`, and `/internal/v1/artifacts/` in the runtime proof script, and rejects removed pre-target route, owner, and DTO terms. |
| Worker claim DTO target metadata | `workers/common/tests/test_api.py::test_claim_analysis_run_accepts_target_selection_snapshot_metadata` proves worker claim responses accept target selection_snapshot metadata fields returned by the live API. |
| Internal artifact download path | `apps/api/internal/api/target_runtime_test.go` and target store tests cover artifact/stored_object resolution, so `/internal/v1/artifacts/{artifact_id}/download-access` can resolve artifacts produced by target workers. |
...
| No-legacy target gate | `bash infra/scripts/no-legacy-target-gate.sh` requires target snippets in `runtime-final-e2e.py`, rejects removed active route/table/DTO terms from target adapter/worker/Web/MCP/runtime paths, rejects pre-target table/column recreation in the target reset migration, and runs `test_target_operations_do_not_reintroduce_compatibility_names`. |
| CopperASR Telegram E2E | `python3 infra/scripts/copper-asr-telegram-e2e.py --json` uses the short voice fixture through a live Telegram-shaped channel account, public upload, inbox collection, current materials surface, atomic processing-run launch, selection_snapshot, analysis_run, transcription worker, and CopperASR runtime. It verifies captured membership detaches at launch, later inbox membership survives delivery, analysis_run_step planning, progress/finalize events, transcript plus policy artifacts, run_manifest provider metadata, artifact download bytes, diagnostics filtering, result_artifact_surface delivery recording, and duplicate delivery prevention. |
...
| CopperASR corrupt-audio classification follow-up | Closed by CopperASR `main` commit `bc3c0da673ee8a7eabb82e2a1530ddf7d5e9bd01`, rebuilt from `external/copper-asr`. The strict live command `python3 infra/scripts/copper-asr-failure-e2e.py --json --require-invalid-audio` records `diagnostic_codes=["asr_invalid_audio"]` for both corrupt and retry runs, publishes only `run_diagnostics` and `run_manifest`, and leaves cancellation at `canceled` with `artifact_count=0`. The manifest no longer accepts `asr_unexpected_runtime_error` for the corrupt fixture. |
| Admin/runtime after target reset | Target runtime services treat missing target rows as normal empty results, expose observability from the target store, and no longer run a separate queue-repair endpoint. Target-reset drift is covered by current target store and API tests. |
| Live admin probe | After API restart against the target-reset compose stack, `/v1/admin/observability` returned HTTP 200 with target observability counters. A fresh log tail after restart had no removed-surface table errors. |
...
| Admin/runtime | `/v1/admin/observability`, `/v1/ws` | Observability semantics and websocket/event stream sanity. | Existing tests and docs; target-reset schema regression proof; live admin observability probe after API restart. | QA runtime audit. |
| Removed public surfaces | No public compatibility routes or execution wrappers remain. | Contract tests and implementation stale scans prove target clients and normal UI use only target API operations. | `no-legacy-target-gate.sh` regression gate. | QA traceability. |
...
| `asr_invalid_audio` | Invalid or unsupported audio must fail the transcription run with analysis_run diagnostics and run policy artifacts, never transcript artifacts or a stuck active card. | worker RED/GREEN regression; `python3 infra/scripts/copper-asr-failure-e2e.py --json` and `python3 infra/scripts/copper-asr-failure-e2e.py --json --require-invalid-audio` cover live corrupt-audio failed run and retry with `asr_invalid_audio`. | Strict invalid-audio proof passes on CopperASR `bc3c0da673ee8a7eabb82e2a1530ddf7d5e9bd01`; no temporary accepted live diagnostic remains in the manifest. |
| `asr_unexpected_runtime_error` | Unexpected CopperASR provider failures must still finalize the run, record ASR diagnostics, and publish only policy artifacts instead of surfacing generic backend-unavailable loops. | worker mapping remains in `workers/common/src/transcriber_workers_common/copper_asr.py` for genuine unexpected provider failures; corrupt-audio live E2E no longer uses this path. | QA audit only. |
...
| Removed names are absent from active contracts and runtime paths. | OpenAPI target-only tests, cleanup stale scans, and the no-legacy regression gate from `media-7f3.10.4`. | QA traceability in `media-7f3.11.1`. |
...
`media-7f3.10.4` has wired the inventory/no-legacy gates so this matrix cannot drift silently, including the observed admin observability and retention drift after target reset.
