...
| CopperASR E2E fixtures are deterministic. | Manifest declares short voice, representative long voice, corrupt audio, cancellation voice, and artifact download cases under `copper_asr_e2e`; `infra/scripts/copper-asr-e2e-harness.py` validates hashes and can copy object-store bytes into a deterministic bucket/key tree; `infra/scripts/copper-asr-failure-e2e.py` consumes the corrupt and cancellation cases against the live stack. | `test_copper_asr_e2e_fixture_manifest_covers_required_inputs_and_hashes`; `test_copper_asr_e2e_harness_reports_deterministic_fixture_plan`; `python3 infra/scripts/copper-asr-failure-e2e.py --json`. | `media-b8s.2.2`, `media-b8s.2.3`, `media-b8s.2.6`, and `media-b8s.2.7` must consume the remaining cases in runtime E2E, inventory, and benchmark proof. |
